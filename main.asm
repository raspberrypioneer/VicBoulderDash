...
reset_grid_of_sprites
    ldx #(12*12)  ;12 rows x 12 columns of sprites on a screen
    lda #$ff
...
skip_null_tile
  inc cache_counter  ;never exceeds 12 row x 12 columns (1 byte)
  ldy map_cols
  iny
  cpy #12  ;12 columns
...
set_cave_colours
  ...
  lda #<_COLOUR_SCREEN_ADDR+96
  sta copy_addr2_low  ;target low
  lda #>_COLOUR_SCREEN_ADDR+96
  sta copy_addr2_high  ;target high

  lda #$40  ;size is 24 lines x 24 characters
  sta clear_size  
  lda #$02
  sta clear_size+1

  lda param_colours+1  ;Group 2 earth
  ora #8  ;clear to param colour
...
clear_screen
  ...
  lda #1  ;clear to colour white
  jmp clear_memory  ;clear target for given size and value
...
display_instructions
  ...
  ldy #0
  sty current_instruction_page+1
...
colour_band_loop
  dey
  sta _COLOUR_SCREEN_ADDR+(24*23),y
  sta _COLOUR_SCREEN_ADDR+(24*24),y
  cpy #0
